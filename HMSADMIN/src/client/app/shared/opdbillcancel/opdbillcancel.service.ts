...
import { opdbilldetailsTS} from '../../opdbillcancel/opdbilldetails';
 ...
 */
@Injectable()
export class OpdbillcancelService {
...
////start here actual opdbillcancel.
getWS_opdbilldetails_bydate_selectjson(inparam0: string,inparam1:string,inparam2:string): Observable<opdbilldetailsTS[]>{
let headers1 = new Headers();

headers1.append('Content-Type', 'application/x-www-form-urlencoded');
var body1 = `input0=${ inparam0}&input1=${ inparam1}&input2=${ inparam2}`;
var url2 = `${this.baseUrl}WS_opdbilldetails_bydate_selectjson`;
let returnval$ = this.http.post(url2 , body1 ,{headers: headers1})
.map((res:Response) => res.json()).catch(this.handleError);
return returnval$;
}

getWS_opdbilldetails_cancel(inparam0: string): Observable<any>{
let headers1 = new Headers();

headers1.append('Content-Type', 'application/x-www-form-urlencoded');
var body1 = `input0=${ inparam0}`;
var url2 = `${this.baseUrl}WS_opdbilldetails_cancel`;
let returnval$ = this.http.post(url2 , body1 ,{headers: headers1})
.map((res:Response) => res.json()).catch(this.handleError);
return returnval$;
}
