...
import {vwopdreceiptprintingTS} from '../../opdreceiptcancelreport/vwopdreceiptprintingTS';
 ...
 */
@Injectable()
export class OpdreceiptcancelreportService {
...
var url2 = `${this.baseUrlreport}WS_OPDReceiptCancelReport`;
...
getreportforopdreceiptcancel(inparam0: string,inparam1: string,inparam2: string): Observable<vwopdreceiptprintingTS[]>{
let headers1 = new Headers();

headers1.append('Content-Type', 'application/x-www-form-urlencoded');
var body1 = `input0=${ inparam1}&input1=${ inparam2}&input2=${inparam0}`;
var url2 = `${this.baseUrl}WS_VwOPDReceiptPrinting_bydate_selectjson`;
let returnval$ = this.http.post(url2 , body1 ,{headers: headers1})
.map((res:Response) => res.json()).catch(this.handleError);
// .map((res:Response) => res.text()).catch(this.handleError);
return returnval$;
