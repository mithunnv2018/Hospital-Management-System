...
import { opddocprescriptionTS } from '../../opdprescription/opdprescription';
import {vwopddocprescriptiondetailsTS} from '../../opdprescription/vwopddocprescriptiondetailsTS';
...
export class  OpdprescriptionService {
  ...
  constructor(private http : Http,private baseurlservice:BaseUrlService){
    this.baseUrl=baseurlservice.getBaseurl()+'opdprescription/';
...
insert(inparam0: string,inparam1: string,inparam2: string): Observable<vwopddocprescriptiondetailsTS>{
let headers1 = new Headers();

headers1.append('Content-Type', 'application/x-www-form-urlencoded');
var body1 = `input0=${ inparam0}&input1=${ inparam1}&input2=${ inparam2}`;
var url2 = `${this.baseUrl}WS_opddocprescription_create`;
...
getall(inparam0: string): Observable<vwopddocprescriptiondetailsTS[]>{
let headers1 = new Headers();

headers1.append('Content-Type', 'application/x-www-form-urlencoded');
var body1 = `input0=${ inparam0}`;
var url2 = `${this.baseUrl}WS_vwopddocprescriptiondetails_selectjson`;
let returnval$ = this.http.post(url2 , body1 ,{headers: headers1})
.map((res:Response) => res.json()).catch(this.handleError);
return returnval$;
}  
...
printreportforopdprescription(inparam0: string): Observable<Reportresult>{
let headers1 = new Headers();

headers1.append('Content-Type', 'application/x-www-form-urlencoded');
var body1 = `param_opddocpid=${ inparam0}`;
var url2 = `${this.baseUrlreport}WS_opdprescriptionreport`;
let returnval$ = this.http.post(url2 , body1 ,{headers: headers1})
.map((res:Response) => res.json()).catch(this.handleError);
