 ...
 */
@Injectable()
export class OpdbillsortbyparticularsreportService {
  ...
  constructor(private http : Http,private baseurlservice:BaseUrlService){
    this.baseUrl=baseurlservice.getBaseurl()+'ipdreceiptcancelreportreport/';
...
printreportforopdbillsortbyparticulars(inparam0: string,inparam1: string,inparam2: string): Observable<Reportresult>{
let headers1 = new Headers();

headers1.append('Content-Type', 'application/x-www-form-urlencoded');
var body1 = `param_hospitalid=${inparam0}&param_fromdate=${ inparam1}&param_todate=${ inparam2}`;
var url2 = `${this.baseUrlreport}WS_OPDBillsortbyparticularsReport`;
...
printreportforopdbillsortbyparticularsdetails(inparam0: string,inparam1: string,inparam2: string): Observable<Reportresult>{
let headers1 = new Headers();

headers1.append('Content-Type', 'application/x-www-form-urlencoded');
var body1 = `param_hospitalid=${inparam0}&param_fromdate=${ inparam1}&param_todate=${ inparam2}`;
var url2 = `${this.baseUrlreport}WS_OPDBillsortbyparticularsDetailReport`;
let returnval$ = this.http.post(url2 , body1 ,{headers: headers1})
.map((res:Response) => res.json()).catch(this.handleError);
// .map((res:Response) => res.text()).catch(this.handleError);
return returnval$;
