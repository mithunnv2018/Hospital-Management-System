...
import { hspreceiptpaymentdetailsTS} from '../../ipdreceiptcancel/ipdreceipt';
 ...
 */
@Injectable()
export class IpdreceiptcancelService {
  ...
  constructor(private http : Http,private baseurlservice:BaseUrlService){
    this.baseUrl=baseurlservice.getBaseurl()+'ipdreceiptcancel/';
...
////start here actual ipdreceiptcancel.
getWS_hspreceiptpaymentdetails_bydate_selectjson(inparam0: string,inparam1:string): Observable<hspreceiptpaymentdetailsTS[]>{
let headers1 = new Headers();

headers1.append('Content-Type', 'application/x-www-form-urlencoded');
var body1 = `input0=${ inparam0}&input1=${ inparam1}`;
var url2 = `${this.baseUrl}WS_hspreceiptpaymentdetails_bydate_selectjson`;
let returnval$ = this.http.post(url2 , body1 ,{headers: headers1})
.map((res:Response) => res.json()).catch(this.handleError);
return returnval$;
}

getWS_hspreceiptpaymentdetails_cancel(inparam0: string): Observable<any>{
let headers1 = new Headers();

headers1.append('Content-Type', 'application/x-www-form-urlencoded');
var body1 = `input0=${ inparam0}`;
var url2 = `${this.baseUrl}WS_hspreceiptpaymentdetails_cancel`;
let returnval$ = this.http.post(url2 , body1 ,{headers: headers1})
.map((res:Response) => res.json()).catch(this.handleError);
return returnval$;
}
