...
import { opdreceiptTS} from '../../opdreceiptcancel/opdreceipt';
 ...
 */
@Injectable()
export class OpdreceiptcancelService {
...
////start here actual opdreceiptcancel.
getWS_opdreceipt_bydate_selectjson(inparam0: string,inparam1:string,inparam2:string): Observable<opdreceiptTS[]>{
let headers1 = new Headers();

headers1.append('Content-Type', 'application/x-www-form-urlencoded');
var body1 = `input0=${ inparam0}&input1=${ inparam1}&input2=${ inparam2}`;
var url2 = `${this.baseUrl}WS_opdreceipt_bydate_selectjson`;
let returnval$ = this.http.post(url2 , body1 ,{headers: headers1})
.map((res:Response) => res.json()).catch(this.handleError);
return returnval$;
}

getWS_opdreceipt_cancel(inparam0: string): Observable<any>{
let headers1 = new Headers();

headers1.append('Content-Type', 'application/x-www-form-urlencoded');
var body1 = `input0=${ inparam0}`;
var url2 = `${this.baseUrl}WS_opdreceipt_cancel`;
let returnval$ = this.http.post(url2 , body1 ,{headers: headers1})
.map((res:Response) => res.json()).catch(this.handleError);
return returnval$;
}
