...
import { MemberService } from '../shared/member/index';
...
export class MemberComponent implements OnInit {
  ...
  clear(){
    ...
    member_type: "", 
    member_contactnos: "", 
    member_emailid: "", member_address: "", comp_id: "", password: "", username: ""            
      }
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: MemberService,
  ...
  onRowSelect(event: any){
        ...
        //  this.member=this.selectedRow;
         let tempproj: Member;
         tempproj = this.selectedRow;
         //Calling WS to get the selected data from server DB of the selected row
      //    this.localService
      // .getselectAllfilemember(tempproj.member_id)
      // .subscribe(
      //    /* happy path */ p => this.fileuploadarr=p,
      //    /* error path */ e => console.log( e),
      //    /* onComplete */ () => console.log('done getselectAllfileuploadmember: ' + this.fileuploadarr));

        //  console.log('onRowSelect: ' + JSON.stringify (this.member));
        //  console.log('onRowSelect fileuploadarr: ' + JSON.stringify (this.fileuploadarr));

         //Calling WS to get the selected data from server DB of the selected row
        this.localService
      .getedit(tempproj.member_id)
      .subscribe(
         /* happy path */ p => this.member=p,
         /* error path */ e => console.log( e),
         /* onComplete */ () => {this.aftercalldate();} );//console.log('done getselectEditmember: ' + this.member));
  ...
  aftercalldate(){
    ...
    // alert(this.member.member_enddate);
    //      alert(new Date(Number(this.member.member_enddate) ));
    //      alert(new Date(Number(this.member.member_startdate) ));
    // this.startdate  = new Date(Number(this.member.member_startdate ));
    // this.enddate  = new Date(Number(this.member.member_enddate ));
  }
  ...
  // startdatechange(){
  //   this.member.projmile_startdate=this.mydate1.parse(this.member.projmile_startdate)
  // }
  // enddatechange(){
  //   this.member.projmile_enddate=this.mydate1.parse(this.member.projmile_enddate)
  // }


  onSubmit(){
      //   alert(JSON.stringify(this.selectedRow));
        //alert(this.member.member_startdate);
        // this.member.member_startdate=this.mydate1.parse(this.startdate)
  
        // this.member.member_enddate=this.mydate1.parse(this.enddate)
        ...
        //  console.log('onSubmit onRowSelect: ' + JSON.stringify (this.member));
        //  console.log('onSubmit onRowSelect fileuploadarr: ' + JSON.stringify (this.fileuploadarr));

        if(this.newmember){//Add new entry saving
           this.localService
          .insert(this.member)
          .subscribe(
            /* happy path */ p => this.members = p,
            /* error path */ e => console.log(e),
...
ngOnInit() { 
      ...
      this.userform = this.fb.group({'member_name':new FormControl('', Validators. required),'drpmembertype':new FormControl('', Validators. required),'member_contactnos':new FormControl('', Validators. required),'member_emailid':new FormControl('', Validators. required),'member_address':new FormControl('', Validators. required),'password':new FormControl('', Validators. required),'username':new FormControl('', Validators. required),});
  ...
  populatedrp(){ //populate dropdowns on page load. by using push and SelectItem
    //console.log(this.drpvar.length);
        ...
        //    this.drpmember.push({label:this.drpvar[i].labeld , value:this.drpvar[i].valued});
        // }

        this.drpmembertype.push({label:"Client" , value:"client"});
        this.drpmembertype.push({label:"Employee" , value:"employee"});
        this.drpmembertype.push({label:"Owner" , value:"owner"});
        
        // alert(JSON.stringify(this.drpmember));
